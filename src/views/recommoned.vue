...
<div style="overflow:hidden;">
  ...
  <div class="recommened">
    <van-swipe class="my-swipe" :autoplay="3000" indicator-color="white">
      <van-swipe-item v-for="item in swipeList" :key="item.id" @click="routeTo(item.targetType,item.targetId)" ><img :src="item.pic"/></van-swipe-item>
    </van-swipe>
      <!-- 推荐歌单 -->
    <div class="tips">推荐歌单</div>
    <div class="sheets">
        <Sheet :type="1" :id="item.id" v-for="item in sheetsList" :key="item.id"  :imgUrl="item.picUrl"
			:name='item.name'
			:count="item.playCount"></Sheet>
    </div>
    <!-- 用户热推 -->
    <div class="tips">{{type[typeCount]}}</div>
    <div class="sheets">
        <Sheet :type="1" :id="item.id" v-for="item in userSheetList" :key="item.id"  :imgUrl="item.coverImgUrl?item.coverImgUrl:item.picUrl"
			:name='item.name'
			:count="item.playcount?item.playcount:item.playCount"></Sheet>
    </div>
    <div class="tips">最新音乐</div>
    <img class="loading" v-if="songList.length == 0" src="../assets/loading.gif" alt="loading">
    <div class="songs">
        <Song  v-for="item in songList" :key="item.id" :id="item.id" :name="item.name" :ftype="item.fee" :artists="item.artists" :album="item.album"></Song>
    </div>
    <div class="footer">
        <div class="ftwrapper" style="position: absolute;left: 0;right: 0;bottom: 0;top: 0;z-index: 1;text-align: center;">
            <div class="logo">
            ...
            <p class="copyright">网易公司版权所有©1997-2020   杭州乐读科技有限公司运营</p>
        </div>
    </div>
  </div>
</div>
...
export default {
  ...
  methods: {
    ...
    async getList () {
      // 最新歌曲
      let res = await this.$http.get('/top/song?type=7')
      if (res.status !== 200) return
      this.songList = res.data.data.slice(0, 10)
       console.log(this.songList)
      //   推荐歌单
      res = await this.$http.get('/personalized?limit=6')
      console.log(res)
      if (res.status !== 200) return
      this.sheetsList = res.data.result
      // 网友精选
      if (this.$store.state.id === 0) {
        this.typeCount = 0
        res = await this.$http.get('/top/playlist?limit=6')
        if (res.status !== 200) return
        this.userSheetList = res.data.playlists
      } else {
        this.userSheetList = []
        this.typeCount = 1
        res = await this.$http.get('/recommend/resource?timestamp=' + new Date().getTime())
        console.log(res)
        if (res.status !== 200) return
        this.userSheetList = res.data.recommend.splice(0, 6)
        if(this.userSheetList.length === 0)
        {
        this.typeCount = 0
        res = await this.$http.get('/top/playlist?limit=6')
        if (res.status !== 200) return
  ...
  async created () {
    let res = await this.$http.get('/banner?type=1')
    console.log(res)
  ...
  async mounted () {
    //  let res = await this.$http.get('/login/refresh')
    //  console.log(res)
    // console.log('请求中')
    // let res = await this.$http.get('/login/cellphone'+(new Date()).valueOf(),{param:{phone:this.phone,md5_password:this.$md5(this.password)}})
    console.log('login')
    // let res = await this.$http.get('/login/status')
    // if(res.status === 200) return console.log('不需要登陆')
    const res = await this.$http.get(`/login/cellphone?phone=${this.phone}&password=${this.password}&timestamp=${new Date().getTime()}`)
    console.log(res)
    if (res.data.code !== 200) return this.$toast.fail('自动登陆失败')
    this.$toast.success('自动登陆成功')
    await this.$store.dispatch('ActionId', res.data.profile.userId)
    await this.$store.dispatch('ActionAvatar', res.data.profile.avatarUrl)
...
.footer{
    ...
    padding-top: 53.3%;
    margin-top: 4px;
    background: url(//s3.music.126.net/mobile-new/img/recommand_bg_2x.png) no-repeat;
